...
@section('content')
    <div class="container-fluid pt-4 px-4">
        ...
        <div class="row mb-4">
            ...
            <div class="col-12 col-md d-flex justify-content-start justify-content-md-end mt-md-0 mt-3">
                <a href="{{ route('school-year.create') }}" class="btn btn-bd-primary d-flex gap-2 align-items-center p-2"
            ...
            <table class="data-table table table-striped table-hover" style="--bs-table-hover-bg: rgba(211, 253, 80, 0.2);">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($school_years as $year)
                        <tr>
                            <th scope="row">{{ $i++ }}</th>
                            <td>{{ $year->school_term }}</td>
                            <td>{{ $year->semester }}</td>
                            <td>
                                <div class="d-flex flex-nowrap">
                                    <a href="{{ route('school-year.edit', $year->slug) }}"
                                    ...
                                    <button class="delete-year btn btn-sm btn-outline-danger border-0"
                                        data-year="{{ $year->slug }}">
    ...
    <div id="import-excel-form" class="d-none d-flex justify-content-center">
        <form action="{{ route('import.school-year') }}" method="POST" enctype="multipart/form-data">
            @csrf
    ...
    <script>
        // Sweet Alert: Delete school year
        const deleteYear = document.querySelectorAll('.delete-year');

        deleteYear.forEach((year) => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                const slug = year.getAttribute('data-year');

                                window.location.href =
                                    "{{ route('school-year.destroy', 'school_year') }}"
                                    .replace('school_year', slug);
                            }
                        });
                    }
                });
