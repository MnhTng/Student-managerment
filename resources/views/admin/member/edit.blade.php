    ...
    <div class="container-fluid pt-4 px-4">
        ...
        <div class="row">
            <div
                ...
                <h4 class="text-danger mb-3">Cập nhật thành viên</h4>

                <form class="needs-validation" method="POST" action="{{ route('member.update', $member->id) }}" novalidate>
                    @csrf
                        ...
                        <div class="col-lg-3 col-12">
                            ...
                            <div class="input-group has-validation">
                                ...
                                <select
                                    ...
                                    <option value="admin" @if ((old('role') ?? $member->role) === 'admin') selected @endif>
                                        Quản trị viên
                                    </option>
                                    <option value="teacher" @if ((old('role') ?? $member->role) === 'teacher') selected @endif>
                                        Giảng viên
                                    </option>
                                    <option value="student" @if ((old('role') ?? $member->role) === 'student') selected @endif>
                                        Sinh viên
                                    </option>
                                </select>
                        ...
                        <div id="identify" class="col-lg-7 col-12">
                            ...
                            <div class="input-group has-validation">
                                ...
                                <input type="text" class="form-control" id="identifier" name="identifier"
                                    list="identifiers" placeholder="Xác định người sở hữu"
                                    value="{{ old('identifier') ?? $member->identifier }}">
                                <datalist id="identifiers">
                                    <option value="{{ $member->identifier }}"
                                        @if (old('identifier')) selected @endif>
                                        {{ $member->name }}
                                    </option>
                                </datalist>
                    ...
                    <div class="d-flex gap-3">
                        ...
                        <a href="{{ route('member.index') }}" class="btn btn-outline-danger rounded-1">
                            {{ __('Cancel') }}
                        </a>
                    </div>
                </form>
        ...
        $(document).ready(function() {
            ...
            $('#role').on('change', function() {
                ...
                if (role !== '') {
                    ...
                    $.ajax({
                        url: "{{ route('member.identify', 'role') }}"
                            .replace('role', role),
                        method: 'GET',
                        data: {
                            role: role
                        ...
                        success: function(data) {
                            ...
                            if (!Array.isArray(data)) {
                                $('#identifier').val('admin').addClass('pe-none');

                                $('#identifiers').append('<option value="' + data + '">' +
                                    data + '</option>');
                            } else {
                                $('#identifier').val('').removeClass('pe-none');

                                data.forEach(identify => {
                                    $('#identifiers').append('<option value="' +
                                        identify.identifier + '">' + identify.name +
                                        '</option>');
                                });
                            }
                        },
            ...
            $('#identifier').on('input', function() {
                const identifier = $(this).val() + '-' + $('#role').val();

                $.ajax({
                    url: "{{ route('member.info', 'identifier') }}"
