...
@section('content')
    <div class="container-fluid pt-4 px-4">
        ...
        <div class="row mb-4">
            ...
            <div class="col-12 col-md d-flex justify-content-start justify-content-md-end mt-md-0 mt-3">
                <a href="{{ route('training-system.create') }}"
                    class="btn btn-bd-primary d-flex gap-2 align-items-center p-2" style="height: fit-content;">
            ...
            <table class="data-table table table-striped table-hover" style="--bs-table-hover-bg: rgba(211, 253, 80, 0.2);">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($training_systems as $training_system)
                        <tr>
                            <th scope="row">{{ $i++ }}</th>
                            <td>{{ $training_system->training_name }}</td>
                            <td>
                                <div class="d-flex flex-nowrap">
                                    <a href="{{ route('training-system.edit', $training_system->training_code) }}"
                                    ...
                                    <button class="delete-system btn btn-sm btn-outline-danger border-0"
                                        data-system="{{ $training_system->training_code }}">
    ...
    <script>
        // Sweet Alert: Delete training system
        const deleteSystem = document.querySelectorAll('.delete-system');

        deleteSystem.forEach((system) => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                const code = system.getAttribute('data-system');

                                window.location.href =
                                    "{{ route('training-system.destroy', 'training_code') }}"
                                    .replace('training_code', code);
                            }
                        });
                    }
                });
        ...
        // Sweet Alert: Add training system
