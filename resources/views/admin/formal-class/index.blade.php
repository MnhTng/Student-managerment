...
@section('content')
    <div class="container-fluid pt-4 px-4">
        ...
        <div class="row mb-4">
            ...
            <div class="col-12 col-md d-flex justify-content-start justify-content-md-end mt-md-0 mt-3">
                <a href="{{ route('formal-class.create') }}" class="btn btn-bd-primary d-flex gap-2 align-items-center p-2"
            ...
            <table class="data-table table table-striped table-hover" style="--bs-table-hover-bg: rgba(211, 253, 80, 0.2);">
                <thead class="table-primary">
                    <tr>
                        ...
                        <th scope="col">{{ __('Academic Advisor') }}</th>
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($formal_classes as $class)
                        <tr>
                            <th scope="row">{{ $i++ }}</th>
                            ...
                            <td>
                                <div class="d-flex flex-nowrap">
                                    <a href="{{ route('formal-class.edit', $class->class_code) }}"
                                    ...
                                    <button class="delete-class btn btn-sm btn-outline-danger border-0"
    ...
    <div id="import-excel-form" class="d-none d-flex justify-content-center">
        <form action="{{ route('import.formal-class') }}" method="POST" enctype="multipart/form-data">
            @csrf
    ...
    <script>
        // Sweet Alert: Delete formal class
        const deleteClass = document.querySelectorAll('.delete-class');

        deleteClass.forEach((formal_class) => {
            formal_class.addEventListener('click', () => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                const code = formal_class.getAttribute('data-class');

                                window.location.href =
                                    "{{ route('formal-class.destroy', 'class_code') }}"
                                    .replace('class_code', code);
                            }
                        });
                    }
                });
